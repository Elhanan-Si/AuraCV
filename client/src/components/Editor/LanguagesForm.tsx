...
export const LanguagesForm: React.FC<LanguagesFormProps> = ({
  ...
  const handleAddLanguage = () => {
    onAdd({
      id: `lang-${Date.now()}`,
      name: '',
      level: language === 'he' ? 'שליטה מלאה / שפת אם' : 'Full Professional'
    });
  };
      ...
      {items.length > 0 ? (
          ...
          {items.map((lang, index) => (
            ...
            >
              ...
              <div className="flex-1 grid grid-cols-2 gap-2">
                <input
                  type="text"
                  placeholder={language === 'he' ? "שפה (לדוגמה: עברית)" : "e.g. Spanish"}
                ...
                <input
                  type="text"
                  placeholder={language === 'he' ? "רמה (לדוגמה: שפת אם)" : "e.g. Native / Fluent"}
                  className={inputClass}
                  value={lang.level || ''}
        ...
        <div className="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-200">
          <p className="text-xs italic text-slate-400">
            {language === 'he' ? "אין שפות רשומות." : "No languages added yet."}
          </p>
        </div>
      )}
